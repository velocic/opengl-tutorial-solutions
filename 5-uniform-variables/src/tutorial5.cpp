...
int main()
{
    ...
    SDL_Window* window = SDL_CreateWindow(
        "ogldev.atspace.co.uk Tutorial 5 -- Uniform Variables",
    ...
    //Query a handle to our scale uniform from the new program object
    GLint scaleHandle = glGetUniformLocation(passthroughProgram, "scale");

    if (scaleHandle == -1) {
        std::cout << "Couldn't locate the uniform variable in the shader program" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    while (userRequestedExit == false)
    {
        ...
        scale += 0.002f;
        glUniform1f(scaleHandle, sinf(scale));
